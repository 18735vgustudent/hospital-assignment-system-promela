/*
CHANGE NOTES: These changes are not included in the report because the code was submitted late (30 minutes before deadline).


 * Time Synchronization: instead of using global channels for time server-client communication, the client process will have its own channel,
which will be sent to server through "timeRegistration" channel along with SUB/UNSUB request.
The server will store these channels and will use them for sending TICK signals & receiving ACK signals
The client will also use its own channel to receive TICK signals & send ACK signals

 * Add "printf log":
    - When customer outside of the hospital is rejected or accepted
    - When customer is ready for surgery in department C
    - When customer is kicked out of Pre-OP room in department C
    - When an OP Room is cleaned by the cleaning team in department C
*/
...
// Channel to send SUB/UNSUB request (along with communication channel).
chan timeRegistration = [N_SUBJECTS] of { mtype:messageType, chan };
...
active proctype ClockTicking() {
    chan subscribedChan[N_SUBJECTS];
    int nSubscribers = 0;
        ...
        :: {
            ...
            do
                :: nempty(timeRegistration) -> {
                    ...
                    if
                        :: reqMsg == SUB -> {
                            subscribedChan[nSubscribers] = reqChan;
                            nSubscribers++;
                        }
                        :: reqMsg == UNSUB -> {
                            i = 0;
                            do
                                ...
                                :: else -> {
                                    if
                                        :: subscribedChan[i] == reqChan -> {
                                            j = i;
                                            do
                                                :: j + 1 >= nSubscribers -> break;
                                                :: else -> {
                                                    subscribedChan[j] = subscribedChan[j + 1];
                                                    j++;
                                                }
                                            od
                                            nSubscribers--;
                                ...
                                :: processingCustomer.dept == A -> {
                                    ...
                                    if
                                        :: globalTime <= TIME_LIMIT - (nWaitingCustomer_DeptA + 1) * avgTreatmentTime_DeptA -> {
                                            ...
                                            printf("\nGateKeeper - A(n) %e customer with id %d, going to Department A, is ACCEPTED at %d\n", temp, processingCustomer.id, globalTime);
                                        ...
                                        :: else -> {
                                            mtype:customerType temp = processingCustomer.type;
                                            printf("\nGateKeeper - A(n) %e customer with id %d, going to Department A, is REJECTED at %d\n", temp, processingCustomer.id, globalTime);
                                            isClosed_DeptA = true;
                                        }
                                    fi
                                }
                                :: processingCustomer.dept == B -> {
                                    ...
                                    if
                                        :: globalTime <= TIME_LIMIT - (nWaitingCustomer_DeptB + 1) * avgTreatmentTime_DeptB -> {
                                            ...
                                            mtype:customerType temp = processingCustomer.type;
                                            printf("\nGateKeeper - A(n) %e customer with id %d, going to Department B, is ACCEPTED at %d\n", temp, processingCustomer.id, globalTime);
                                        }
                                        :: else -> {
                                            mtype:customerType temp = processingCustomer.type;
                                            printf("\nGateKeeper - A(n) %e customer with id %d, going to Department B, is REJECTED at %d\n", temp, processingCustomer.id, globalTime);
                                            isClosed_DeptB = true;
                                        }
                                    fi
                                }
                                :: processingCustomer.dept == C -> {  // Checking rejection for department C
                                    if
                                        :: globalTime <= TIME_LIMIT - (nWaitingCustomer_DeptC + 1) * avgTreatmentTime_DeptC -> {
                                            ...
                                            mtype:customerType temp = processingCustomer.type;
                                            printf("\nGateKeeper - A(n) %e customer with id %d, going to Department C, is ACCEPTED at %d\n", temp, processingCustomer.id, globalTime);
                                        }
                                        :: else -> {  // Reject
                                            mtype:customerType temp = processingCustomer.type;
                                            printf("\nGateKeeper - A(n) %e customer with id %d, going to Department C, is REJECTED at %d\n", temp, processingCustomer.id, globalTime);
                                            isClosed_DeptC = true;
                                        }
                                    fi
                                    skip;
                ...
                :: myChan ? TICK -> {
                    ...
                    if
                        :: preOPTime == 0 -> {
                            ...
                            printf("\nC - A(n) %e customer with id %d is ready for surgery at %d\n", preOPCustomerType, preOPCustomerID, globalTime)
                        ...
                        :: preOPCustomerType != VIP -> {
                            if 
                                :: nempty(deptVIPQueue_C) -> {
                                    ...
                                    printf("\nC - A(n) %e customer with id %d has been kicked from the Pre-OP Room by VIP customer %d at %d\n", preOPCustomerType, preOPCustomerID, vipId, globalTime)
                ...
                :: myChan ? TICK -> {
                    ...
                    if
                        :: cleaningTime == 0 -> {
                            ...
                            printf("\nC - OP Room %d is cleaned by the cleaning time, from %d to %d (%d minutes).\n", cleaningRoomId, globalTime-cleaningTimeSave, globalTime, cleaningTimeSave);
